...
use egg::{Id, Pattern, RecExpr, StopReason, Subst, Symbol, SymbolLang, Var};

use crate::adapter::{EGraph, Extractor, RunnerConfig};
...
pub struct TheSy<G: EGraph> {
    ...
    searchers: HashMap<
        String,
        (
            multidiff::MultiDiffSearcher,
            Vec<(Var, RecExpr<SymbolLang>)>,
        ),
    >,
...
/// *** TheSy Statics ***
impl<G: EGraph> TheSy<G> {
    fn replace_ops(
        exp: &RecExpr<SymbolLang>,
        replacments: &HashMap<Symbol, Symbol>,
    ) -> RecExpr<SymbolLang> {
        RecExpr::from(
            exp.as_ref()
                .iter()
                .map(|s| {
                    replacments
                        .get(&s.op)
                        .map(|x| SymbolLang::new(x.as_str(), s.children.clone()))
                        .unwrap_or(s.clone())
                })
                .collect_vec(),
        )
    ...
    fn create_sygue_serchers<'a>(
        dict: &[Function],
        datatypes: impl Iterator<Item = &'a DataType>,
    ) -> HashMap<
        String,
        (
            multidiff::MultiDiffSearcher,
    ...
    > {
        ...
        Self::known_functions(&datas, dict).for_each(|fun| {
            if !fun.params.is_empty() {
                let params: Vec<(Var, RecExpr<SymbolLang>)> = fun
                    .params
                    .iter()
                    .enumerate()
                    .map(|(i, t)| (Var::from_str(&*format!("?param_{}", i)).unwrap(), t.clone()))
                    .collect_vec();
                let patterns = params
                    .iter()
                    .flat_map(|(v, typ)| {
                ...
                res.insert(
                    fun.name.clone(),
                    (multidiff::MultiDiffSearcher::new(patterns), params),
                );
            }
        });
        res
    ...
    pub fn new(datatype: DataType, examples: Examples, dict: Vec<Function>) -> TheSy<G> {
        Self::new_with_ph(
            vec![datatype.clone()],
            HashMap::from_iter(iter::once((datatype, examples))),
            dict,
            2,
            None,
        )
    ...
    pub fn new_with_ph(
        ...
        lemmas: Option<
            Vec<(
                HashMap<RecExpr<SymbolLang>, RecExpr<SymbolLang>>,
                Option<RecExpr<SymbolLang>>,
                RecExpr<SymbolLang>,
                RecExpr<SymbolLang>,
            )>,
        ...
        let datatype_to_prover: HashMap<DataType, Prover> = datatypes
            .iter()
            .map(|d| (d.clone(), Prover::new(d.clone())))
            .collect();
        let (mut egraph, mut example_ids) =
            Self::create_graph_example_ids(&datatypes, &examples, &dict, ph_count);

        let apply_rws = Self::create_apply_rws(&dict, &datatypes, ph_count);
        let system_rws: Vec<Rewrite> = apply_rws
            .into_iter()
            ...
            v.into_iter()
                .map(|(vars, precond, ex1, ex2)| {
                    ...
                    for v in vars {
                        ...
                        types_to_vars.get_mut(&v.1).unwrap().insert(
                            v.0.as_ref().last().unwrap().op,
                            Self::get_ph(&v.1, current_ph),
                        );
                    }
                    let replacments: HashMap<Symbol, Symbol> = types_to_vars
                        .values()
                        .flat_map(|v| v.iter().map(|(k, f)| (*k, Symbol::from(&f.name))))
                        .collect();
                    ...
                    types_to_vars
                        ...
                        .flat_map(|typ| {
                            types_to_vars[typ]
                                .values()
                                .zip(iter::once(typ).cycle())
                                .map(|(ph, typ)| {
                                    (
                                        precond_replaced.clone().map(|p| {
                                            Self::replace_op(
                                                &p,
                                                Symbol::from(&ph.name),
                                                Symbol::from(&Self::get_ph(typ, 0).name),
                                            )
                                        }),
                                        Self::replace_op(
                                            &ex1_replaced,
    ...
    fn known_functions<'a>(
        datatypes: &'a Vec<DataType>,
        dict: &'a [Function],
    ...
    fn create_graph_example_ids(
        ...
        G,
        HashMap<DataType, HashMap<Id, Vec<Id>, RandomState>, RandomState>,
    ) {
            ...
            .chain(datatypes.iter().flat_map(|d| d.constructors.iter()))
        {
            ...
            for d in datatypes.iter() {
                example_ids.get_mut(d).unwrap().insert(
                    id,
                    Vec::from_iter(iter::repeat(id).take(examples[d].examples().len())),
                );
            }
        }
        ...
        for d in datatypes.iter() {
            ...
            let initial_example_ids = examples[d]
                .examples()
                .iter()
        ...
        let apply_rws = dict
            ...
            .map(|fun| {
                let params = &fun
                    ...
                    .collect_vec();
                let name = &fun.name;
                let searcher: Pattern<SymbolLang> = format!(
                    "(apply {} {})",
                    name,
                    params
                        .iter()
                ...
                .parse()
                .unwrap();
                let applier: Pattern<SymbolLang> = format!(
                    "({} {})",
                    name,
                    params
                        .iter()
    ...
    fn collect_phs<'a>(
        known_functions: impl Iterator<Item = &'a Function>,
        ph_count: usize,
    ) -> Vec<Function> {
...
impl<G: EGraph> TheSy<G> {
    fn extract_classes(&self) -> HashMap<Id, (RepOrder, RecExpr<SymbolLang>)> {
        let mut ext = self.egraph.extractor();
        // each datatype should have the same keys
        let keys: HashSet<Id> = self
            .example_ids
            .iter()
            .flat_map(|(d, m)| m.keys())
            .copied()
        ...
        keys.iter()
            .map(|key| {
                ...
                (*updated_key, ext.find_best(*updated_key).unwrap())
            })
            .collect()
    }
    ...
    fn fix_example_ids(&mut self) {
        ...
        for d in self.example_ids.keys() {
            ...
            let t = self.example_ids[d]
                .iter()
                .map(|(k, v)| {
                    (
                        self.egraph.find(*k),
                        v.iter().map(|x| self.egraph.find(*x)).collect(),
                    )
                })
                .collect();
    ...
    pub fn increase_depth(&mut self) {
        ...
        fn create_edge(
            op: &String,
            params: &Vec<(Var, RecExpr<SymbolLang>)>,
            sub: &Subst,
        ) -> SymbolLang {
            SymbolLang::new(
                op.clone(),
                params
                    .iter()
                    .map(|(v, typ)| sub.get(v.clone()).unwrap())
                    .copied()
                    .collect(),
            )
        }
        ...
        fn translate_edge(
            edge: &SymbolLang,
            e_index: usize,
            translations: &HashMap<Id, Vec<Id>>,
        ) -> SymbolLang {
            let new_child = edge
                .children
                .iter()
                .map(|id| translations[id][e_index])
                .collect_vec();
            ...
            .map(|(op, (searcher, params))| {
                (
                    ...
                    searcher
                        .search(&self.egraph)
                        .iter_mut()
                        .flat_map(|sm| std::mem::take(&mut sm.substs))
                        .collect_vec(),
                )
            })
            .collect_vec();
        ...
        for (op, params, subs) in op_matches {
            let typ = {
                let fun = &self
                    .dict
                    .iter()
                    .chain(self.datatypes.keys().flat_map(|d| d.constructors.iter()))
                    .find(|f| &f.name == op)
                    .unwrap();
            ...
            for sub in subs {
                ...
                self.egraph
                    .add(SymbolLang::new("typed", vec![key, type_key]));
                ...
                for d in self.datatypes.keys() {
                    ...
                    for i in 0..self.example_ids[d].iter().next().unwrap().1.len() {
                        new_ids.push(self.egraph.add(translate_edge(
                            &new_edge,
                            i,
                            &self.example_ids[d],
                        )));
        ...
        self.egraph.rebuild();
        self.stats
            .update_term_creation(key, self.egraph.total_number_of_nodes());
    }
    ...
    pub fn get_conjectures(
        ...
        self.fix_example_ids();
        let finer_classes: HashMap<DataType, HashMap<Vec<Id>, HashSet<Id>>> = self
            .example_ids
            .iter()
            .map(|(d, m)| {
                ...
                for (id, vals) in m {
                    if !finer_equality_classes.contains_key(vals) {
                        finer_equality_classes.insert(
                            vals.iter().map(|i| self.egraph.find(*i)).collect_vec(),
                            HashSet::new(),
                        );
                    }
                    finer_equality_classes
                        .get_mut(vals)
                        .expect("Should have just added if missing")
                        .insert(self.egraph.find(*id));
                }
        ...
        for (d, m) in finer_classes {
            for set in m.values() {
                ...
                for couple in choose(&set.iter().collect_vec()[..], 2) {
                    let max_cop = if reps[couple[0]].0 > reps[couple[1]].0 {
                        (reps[couple[0]].0.clone(), reps[couple[1]].0.clone())
                    } else {
                        (reps[couple[1]].0.clone(), reps[couple[0]].0.clone())
                    };
                    res.push((
                        max_cop,
                        reps[couple[0]].1.clone(),
                        reps[couple[1]].1.clone(),
                        d.clone(),
                    ));
                }
            }
    ...
    pub fn check_equality(
        rules: &[Rewrite],
        precond: &Option<RecExpr<SymbolLang>>,
    ...
    fn check_goals(
        &mut self,
        case_splitter: &mut Option<&mut CaseSplit<G>>,
        rules: &mut [Rewrite],
    ) -> Option<
        Vec<(
            Option<Pattern<SymbolLang>>,
            Pattern<SymbolLang>,
            Pattern<SymbolLang>,
            Rewrite,
        )>,
            ...
            for (precond, ex1, ex2) in conjs {
                for p in self.datatypes.values() {
                    ...
                    if res.is_some() {
                        ...
                        if cfg!(feature = "stats") {
                            self.stats.failed_proofs_goals.push((
                                ex1.pretty(500),
                                ex2.pretty(500),
                                SystemTime::now().duration_since(start.unwrap()).unwrap(),
                            ));
    ...
    pub fn run(
        ...
        mut case_spliter: Option<CaseSplit<G>>,
        max_depth: usize,
    ) -> Vec<(
        Option<Pattern<SymbolLang>>,
        Pattern<SymbolLang>,
        ...
        info!(
            "Running TheSy on datatypes: {} dict: {}",
            self.datatypes.keys().map(|x| &x.name).join(" "),
            self.dict.iter().map(|x| &x.name).join(" ")
        );
        self.stats.init_run();
        ...
        for depth in 0..max_depth {
            ...
            if !cfg!(feature = "no_expl_split") {
                if splitter_to_use.is_some() {
                    if self.prove_case_split_rules(
                        splitter_to_use.unwrap(),
                        rules,
                        &mut found_rules,
                        new_rules_index,
                    ) {
            ...
            'outer: while !conjectures.is_empty() {
                ...
                let mut new_rules =
                    self.datatypes[&d].prove_ind(&mut splitter_to_use, &rules, &ex1, &ex2);
                if new_rules.is_some() {
                    let temp = new_rules;
                    new_rules = self.datatypes[&d].generalize_prove(
                        &mut splitter_to_use,
                        &rules,
                        &ex1,
                        &ex2,
                    ...
                    if new_rules.is_none() {
                        ...
                        ex1 = new_rules.as_ref().unwrap()[0]
                            .1
                            .pretty_string()
                            .parse()
                            .unwrap();
                        ex2 = new_rules.as_ref().unwrap()[0]
                            .2
                            .pretty_string()
                            .parse()
                            .unwrap();
    ...
    fn prove_case_split_rules(
        &mut self,
        case_splitter: &mut CaseSplit<G>,
        rules: &mut Vec<Rewrite>,
        found_rules: &mut Vec<(
            Option<Pattern<SymbolLang>>,
            Pattern<SymbolLang>,
        ...
        for (o, mut ex1, mut ex2, d) in conjs_before_cases.into_iter().rev() {
            if conjectures
                .iter()
                .any(|(_, other_ex1, other_ex2, _)| other_ex1 == &ex1 && &ex2 == other_ex2)
            {
                continue;
            }
            if Self::check_equality(&rules[..], &None, &ex1, &ex2) {
            ...
            let mut new_rules =
                self.datatypes[&d].prove_all(&mut splitter_to_use, rules, &ex1, &ex2);
            if new_rules.is_none() {
                continue;
            } else {
                let temp = new_rules;
                new_rules =
                    self.datatypes[&d].generalize_prove(&mut splitter_to_use, &rules, &ex1, &ex2);
                if new_rules.is_none() {
                    new_rules = temp;
                } else {
                    ex1 = new_rules.as_ref().unwrap()[0]
                    ...
                    ex2 = new_rules.as_ref().unwrap()[0]
                        ...
                        .unwrap();
                    warn!(
                        "generalized case_split proof as: {} -- {}",
                        ex1.pretty(500),
                        ex2.pretty(500)
                    );
                }
...
mod test {
    ...
    fn create_list_sygue<G: EGraph>() -> TheSy<G> {
        TheSy::<G>::new(
            create_list_type(),
    ...
    fn no_double_translation<G: EGraph>() {
    ...
    fn test_creates_expected_terms_nat<G: EGraph>() {
    ...
    fn check_conjectures_sane<G: EGraph>() {
        let mut syg = create_nat_sygue::<G>();
        let mut rewrites = create_pl_rewrites();
    ...
    fn create_filter_p_filter_q<G: EGraph>() {
    ...
    fn filter_p_filter_q_conjecture<G: EGraph>() {
        ...
        let mut thesy = TheSy::<G>::from(&filter_defs);
        ...
        let mut case_splitter = TheSy::<G>::create_case_splitter(filter_defs.case_splitters);
    ...
    fn test_ite_split_rule<G: EGraph>() {
        let mut egraph: G = G::default();
        egraph.add_expr(&RecExpr::from_str("(ite z x y)").unwrap());
        egraph.rebuild();
        let mut case_splitters: CaseSplit<G> = consts::system_case_splits();
