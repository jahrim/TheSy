pub mod multisearcher {
    ...
    pub trait AsSearcher { fn as_searcher(self) -> Searcher; }
    pub trait HasSearchEClass { fn search_eclass<G: EGraph>(&self, egraph: &G, eclass: Id) -> Option<SearchMatches>; }
    pub trait HasSearch { fn search<G: EGraph>(&self, egraph: &G) -> Vec<SearchMatches>; }
    pub trait HasVars { fn vars(&self) -> Vec<Var>; }
    ...
    macro_rules! Searcher {
        ({ $($variant:ident($type:ty)),* $(,)? }) => {
            ...
            impl HasSearchEClass for Searcher {
                fn search_eclass<G: EGraph>(&self, egraph: &G, eclass: Id) -> Option<SearchMatches> {
                    match self { $(Self::$variant(searcher) => searcher.search_eclass(egraph, eclass)),* }
                }
            }
            impl HasSearch for Searcher {
                fn search<G: EGraph>(&self, egraph: &G) -> Vec<SearchMatches> {
    ...
    impl crate::egg::Searcher<SymbolLang, ()> for Searcher {
        ...
        fn search_eclass(&self, egraph: &egg::EGraph<SymbolLang, ()>, eclass: Id) -> Option<SearchMatches> {
    ...
    pub mod pattern {
        ...
        impl AsSearcher for &str {
            fn as_searcher(self) -> Searcher {
                self.parse::<egg::Pattern<SymbolLang>>().unwrap().as_searcher()
            }
        }
        ...
        impl HasVars for egg::Pattern<SymbolLang> {
            fn vars(&self) -> Vec<Var> { egg::Pattern::vars(self) }
        }
    }
    pub mod either {
        use super::*;
        pub struct EitherSearcher {
            node: Rc<Either<Searcher, Searcher>>,
        }

        impl EitherSearcher {
            pub fn left(left: impl AsSearcher) -> Self { EitherSearcher { node: Rc::new(Either::Left(left.as_searcher())) } }
            pub fn right(right: impl AsSearcher) -> Self { EitherSearcher { node: Rc::new(Either::Right(right.as_searcher())) } }
        }
    ...
    pub mod multieq {
        ...
        impl AsSearcher for MultiEqSearcher {
            fn as_searcher(self) -> Searcher { Searcher::MultiEq(self) }
        }
        impl HasSearchEClass for MultiEqSearcher {
            fn search_eclass<G: EGraph>(&self, _: &G, _: Id) -> Option<SearchMatches> {
                unimplemented!()
            }
        }
    ...
    pub mod multidiff {
        ...
        impl AsSearcher for MultiDiffSearcher {
            fn as_searcher(self) -> Searcher { Searcher::MultiDiff(self) }
        }
    ...
    pub mod filter {
        ...
        pub type Filter = Rc<dyn Fn(&dyn Searchable, Vec<SearchMatches>) -> Vec<SearchMatches>>;
        ...
        impl FilterSearcher {
            pub fn new(searcher: impl AsSearcher, filter: Filter) -> Self {
                FilterSearcher { searcher: Rc::new(searcher.as_searcher()), filter }
            }
            pub fn combine_filters(fs: Vec<Filter>) -> Filter {
                Rc::new(move |searchable, mut original| { 
            ...
            pub fn exist_filter(searcher: impl AsSearcher, root: Var, existence: bool) -> Filter {
                ...
                Rc::new(move |searchable, original| {
        ...
        impl HasVars for FilterSearcher {
            fn vars(&self) -> Vec<Var> { self.searcher.vars() }
        }
        ...
        impl PrettyString for FilterSearcher {
            fn pretty_string(&self) -> String { self.searcher.pretty_string() }
        }
    }
    pub mod pointer {
        use super::*;
        ...
        impl PointerSearcher {
            pub fn new(searcher: impl AsSearcher) -> Self { 
                PointerSearcher { searcher: Rc::new(searcher.as_searcher()) } 
            }
        }
        impl AsSearcher for PointerSearcher {
            fn as_searcher(self) -> Searcher { Searcher::Pointer(self) }
        }
...
mod tests {
    ...
    use crate::{adapter::{EGraph, Egg}, eggstentions::searchers::multisearcher::*};
