...
handle() { 
  ...
  echo "$csv_row" > $OUTDIR/tmp/results.$num.csv
}
export -f handle
parallel --ungroup -j $JOBS --joblog $OUTDIR/tmp/parallel.log --resume handle {} "{#}" ::: $BENCHMARK_FILES
cat $OUTDIR/tmp/results.*.csv >> $OUTDIR/csv/results.csv

# Cleanup
echo "Cleaning up temporary files..."
rm -rf $OUTDIR/tmp

echo "TheSy evaluation completed. Results saved to $OUTDIR/csv/results.csv"